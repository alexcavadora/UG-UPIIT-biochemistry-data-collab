...
def main():
    ...
    if CSV.exists():
       ...
       ds = pd.read_csv(CSV)
       try:
           tmax = float(ds["time"].max()) if "time" in ds.columns else float("nan")
       except Exception:
           tmax = float("nan")
    ...
    if "time" in ds.columns:
       writeln(f"sample time span (time column): min={ds['time'].min()}, max={ds['time'].max()}")
    else:
       writeln("No 'time' column in dataset")
    if "MeasurementHours" in ds.columns:
       writeln(f"measurement time span (MeasurementHours): min={ds['MeasurementHours'].min()}, max={ds['MeasurementHours'].max()}")
    ...
    for f in sorted(ds["fermentation"].unique()):
       ...
       meas_min = sub["MeasurementHours"].min() if "MeasurementHours" in sub.columns else "NA"
       meas_max = sub["MeasurementHours"].max() if "MeasurementHours" in sub.columns else "NA"
       writeln(f"fermentation {f}: sample time min={samp_min}, max={samp_max}; measurement min={meas_min}, max={meas_max}")
    writeln("\n--- DIELECTRIC STATS ---")
    writeln("Per-frequency diagnostics temporarily disabled (frequency logging turned off).")
    Path(OUT).write_text("\n".join(lines))
